...
class TestIssueGenerationMethods(unittest.TestCase):

	def test_create_issue_body(self):
		sample_payload = json.loads(file('sample_payload.json').read())
		sample_commit = sample_payload['commits'][0]
		
		expected_body = u"Autogenerated commit via ShipShit. Commit bd2f1d4c4e18c830bdcda756c1ca8cf19c99802b: 'shipshit'"
		generated_body = GithubClient.create_issue_body(sample_commit)

		self.assertEqual(expected_body, generated_body)
	def test_create_issue_body_verify_emoji(self):
		sample_payload = json.loads(file('sample_payload.json').read())
		sample_commit = sample_payload['commits'][0]
		sample_commit['message'] = u'blah 🚀💩'
		expected_body = u"Autogenerated commit via ShipShit. Commit bd2f1d4c4e18c830bdcda756c1ca8cf19c99802b: 'blah 🚀💩'"
		generated_body = GithubClient.create_issue_body(sample_commit)
